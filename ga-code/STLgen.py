...
def plot(filename):
    ...
    plt.show()
def combine(filename1,filename2,filename3,final_file):
    main_body = mesh.Mesh.from_file(filename1)
    addendum1 = mesh.Mesh.from_file(filename2)
    addendum2 = mesh.Mesh.from_file(filename3)

    combined = mesh.Mesh(np.concatenate([main_body.data, addendum1.data,addendum2.data]))
